 ...
 * - 0: APPROVE (0x095ea7b3)
 * - 1: SET_APPROVAL_FOR_ALL (0xa22cb465)
 * - 2: TRANSFER (0x23b872dd)
 * - 3: SAFE_TRANSFER (0x42842e0e)
 * - 4: SAFE_TRANSFER_DATA (0xb88d4fde)
 */
...
static const uint8_t *const ERC721_SELECTORS[NUM_ERC721_SELECTORS] = {
...
static int fuzz_erc721_plugin(const uint8_t *data, size_t size) {
    ...
    // Use first byte to select which ERC721 selector to test
    uint8_t selector_idx = data[0] % NUM_ERC721_SELECTORS;
    const uint8_t *selector = ERC721_SELECTORS[selector_idx];
    ...
    erc721_plugin_call(ETH_PLUGIN_INIT_CONTRACT, &init_contract);
    if (init_contract.result != ETH_PLUGIN_RESULT_OK) {
        return 0;
    }

    // Provide parameters - ERC721 expects specific parameter offsets
    size_t offset = 0;
    if (size >= sizeof(txContent_t)) {
    ...
    while (size - offset >= PARAMETER_LENGTH) {
        ...
        erc721_plugin_call(ETH_PLUGIN_PROVIDE_PARAMETER, &provide_param);
    ...
    erc721_plugin_call(ETH_PLUGIN_FINALIZE, &finalize);
    ...
    if (finalize.tokenLookup1 || finalize.tokenLookup2) {
        ...
        erc721_plugin_call(ETH_PLUGIN_PROVIDE_INFO, &provide_info);
    ...
    erc721_plugin_call(ETH_PLUGIN_QUERY_CONTRACT_ID, &query_id);
    ...
    for (uint8_t i = 0; i < total_screens; i++) {
        ...
        erc721_plugin_call(ETH_PLUGIN_QUERY_CONTRACT_UI, &query_ui);
